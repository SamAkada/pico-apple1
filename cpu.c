...
#ifdef BASIC
  if (address == 0xF004 || address == 0xD010) { //EhBASIC simulated,Woz Monitor ASIC input
    tempval = getkey();
    clearkey();
    return(tempval);
  }
...
#else
  if (address == 0xD011) { //Woz Monitor Keyboard input
		if(getkey() >= 0x80){
			return 0xA7;
		}else{
			return 0x00;
...
void write6502(uint16_t address, uint8_t value) {
  ...
  #else
  if (address == 0xD012 ) { //Woz Monitor Display output
	//printf("%02X\n",value);
	if((value&0x7F) == 0x0D ){
		printf("\n");
	}else if (value == 0xDF ){
